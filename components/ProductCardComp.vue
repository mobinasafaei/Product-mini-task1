<template>
    ...
    <div
      class="border m-2 p-2 h-full rounded flex flex-col justify-between leading-normal"
    >
      <div class="mb-8">
        <p class="text-gray-900 font-bold text-xl mb-2">{{ product.attributes.name }}</p>
        <p class="text-gray-700 text-base line-clamp-3 text-ellipsis">{{ product.attributes.description }}</p>
      </div>
